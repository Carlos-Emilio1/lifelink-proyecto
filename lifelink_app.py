...
# ==========================================
# 1. LÓGICA DE NEGOCIO (COMPATIBILIDAD SANGRE)
# ==========================================
...
# ==========================================
# 2. DEFINICIÓN DE PLANTILLAS (REORDENADO PARA EVITAR NAMEERROR)
# ==========================================
        ...
        <div class="lg:grid lg:grid-cols-12 lg:gap-12 items-center">
            <div class="sm:text-center lg:col-span-6 lg:text-left">
                ...
                <p class="text-lg text-slate-500 max-w-lg leading-relaxed mb-10 font-bold italic">
                    Gestión blindada de hemoderivados, fármacos controlados y logística médica especializada bajo estricta normativa de salud.
                </p>
...
{% extends "base.html" %}
{% block content %}
<div class="max-w-5xl mx-auto py-12 px-4 italic">
    <div class="bg-white rounded-[4rem] shadow-2xl p-16 border border-slate-50 shadow-blue-50/50">
        ...
        <p class="text-[11px] font-black text-slate-300 uppercase tracking-widest mb-16">Audit System TechPulse v.4.0 enabled</p>
        
        <form method="POST" enctype="multipart/form-data" class="space-y-12">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-12 text-left">
                <div>
                    <label class="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-6 italic">Documentación Gráfica (Foto Real)</label>
                ...
                <div class="space-y-10">
                    ...
                    <!-- CONTROL DE CADUCIDAD: CAMPO OBLIGATORIO PARA EL 100 -->
                    <div class="p-8 bg-amber-50 rounded-[2.5rem] border border-amber-100 shadow-inner">
                        <label class="block text-[10px] font-black text-amber-600 uppercase tracking-widest mb-4 italic italic"><i class="fas fa-calendar-check mr-2"></i> Control de Caducidad Biológica</label>
                        ...
                        <p class="text-[8px] text-amber-400 mt-4 font-bold uppercase tracking-widest leading-relaxed italic">Filtro Automático: El recurso se ocultará automáticamente al alcanzar esta fecha por seguridad biológica.</p>
                    </div>
                </div>
            </div>
            ...
            <div class="space-y-6">
                ...
                <div class="bg-blue-50 p-8 rounded-[3rem] border border-blue-100 flex items-center gap-6 shadow-inner text-brand italic">
                    ...
                    <input type="text" id="direccion_text" name="direccion_manual" readonly placeholder="Detectando geolocalización de transferencia..." class="bg-transparent w-full outline-none text-[12px] font-black uppercase tracking-widest italic">
                </div>
                <input type="hidden" id="lat" name="lat">
                <input type="hidden" id="lng" name="lng">
            </div>

            <button type="submit" class="w-full btn-medical py-8 rounded-[3.5rem] font-black text-4xl shadow-2xl shadow-blue-200 italic uppercase hover:scale-[1.01] transition-all">Certificar Recurso</button>
        </form>
    </div>
</div>
...
# Loader de Plantillas Actualizado (CORREGIDO ERROR NAMEERROR)
app.jinja_loader = jinja2.DictLoader({
    'base.html': base_template,
    'home.html': home_template,
    'dashboard.html': dashboard_template,
    'soporte.html': """{% extends "base.html" %}{% block content %}<div class="max-w-5xl mx-auto py-16 px-4 text-center"><h2 class="text-6xl font-black mb-12 tracking-tighter uppercase italic leading-none">Pilar Legal y <span class="text-brand text-4xl">Compliance.</span></h2><div class="grid grid-cols-1 md:grid-cols-2 gap-10 mb-20 text-left uppercase italic font-black"><div class="bg-white p-12 rounded-[4rem] border border-slate-100 shadow-2xl"><h3 class="text-2xl font-black mb-6 flex items-center gap-3"><i class="fas fa-scale-balanced text-brand"></i> Legal Tech</h3><p class="text-[10px] text-slate-500 leading-relaxed tracking-widest uppercase">Bloqueamos la monetización de sangre según NOM-253. Auditamos recetas para fármacos y filtramos automáticamente recursos caducados bajo el estándar LFPDPPP.</p></div><div class="bg-slate-900 p-12 rounded-[4rem] text-white shadow-2xl"><h3 class="text-2xl font-black mb-6 flex items-center gap-3"><i class="fas fa-coins text-amber-400"></i> Monetización B2B</h3><p class="text-[10px] text-slate-400 font-medium leading-relaxed tracking-widest uppercase">Suscripciones premium para instituciones de salud ($990 MXN/mes) y tarifas logísticas por gestión de transporte pesado certificado COFEPRIS.</p></div></div><div class="bg-white p-12 rounded-[4rem] border border-blue-100 shadow-2xl max-w-2xl mx-auto"><h3 class="text-2xl font-black mb-8 italic uppercase">Reporte Legal TechPulse</h3><form action="{{ url_for('enviar_soporte') }}" method="POST" class="space-y-4"><input name="asunto" placeholder="Motivo Jurídico" required class="w-full p-5 bg-slate-50 rounded-2xl border-none outline-none font-black text-xs italic"><textarea name="mensaje" placeholder="Evidencia del reporte..." rows="4" required class="w-full p-5 bg-slate-50 rounded-2xl border-none outline-none font-bold text-sm italic"></textarea><button class="w-full btn-medical py-6 text-[10px] font-black uppercase tracking-widest shadow-2xl italic">Certificar Reporte</button></form></div></div>{% endblock %}""",
    'search.html': """{% extends "base.html" %}{% block content %}<div class="max-w-7xl mx-auto py-12 px-4 flex flex-col lg:flex-row gap-16"><div class="lg:w-80"><div class="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-2xl sticky top-24"><h4 class="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-8 italic">Malla de Búsqueda</h4><form method="GET" class="space-y-4"><input name="q" placeholder="Ej: Sangre O+..." class="w-full p-4 bg-slate-50 border-none rounded-2xl text-[10px] font-black outline-none shadow-inner italic uppercase"><button class="w-full btn-medical py-4 text-[10px] font-black uppercase tracking-widest italic shadow-xl">Actualizar Red</button></form></div></div><div class="flex-1 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-10">{% for item in resultados %}<div class="bg-white rounded-[3rem] border border-slate-50 shadow-sm overflow-hidden hover:shadow-2xl transition-all duration-1000 group relative">{% if item.urgente %}<div class="absolute z-20 top-6 left-6 bg-red-600 text-white text-[7px] px-3 py-1 rounded-full font-black tracking-widest shadow-xl animate-pulse italic uppercase italic">Urgencia P1</div>{% endif %}<img src="{{ item.imagen_url }}" class="w-full h-72 object-cover group-hover:scale-110 transition-transform duration-1000 shadow-inner grayscale group-hover:grayscale-0"><div class="p-8"><h3 class="font-black text-slate-800 text-2xl tracking-tighter uppercase italic mb-1">{{ item.nombre }}</h3><p class="text-[9px] text-brand font-black uppercase tracking-[0.1em] mb-6 italic">{{ item.categoria }}</p><p class="text-[8px] font-black text-red-400 uppercase mb-8 italic tracking-widest italic">Vence: {{ item.fecha_caducidad }}</p><div class="flex justify-between items-center"><span class="text-3xl font-black text-slate-900 tracking-tighter italic">{% if item.tipo_publicacion == 'Venta' %} ${{ item.precio }} {% else %} GRATIS {% endif %}</span><a href="{{ url_for('confirmar_compra', id=item.id_oferta_insumo) }}" class="w-14 h-14 bg-blue-50 text-brand rounded-[1.5rem] flex items-center justify-center hover:bg-brand hover:text-white transition-all shadow-xl shadow-blue-50 shadow-inner"><i class="fas fa-chevron-right text-lg"></i></a></div></div></div>{% endfor %}</div></div>{% endblock %}""",
    'publish.html': publish_template,
    'checkout.html': """{% extends "base.html" %}{% block content %}<div class="max-w-2xl mx-auto py-20 px-4 text-center animate-in zoom-in-95 duration-700 uppercase italic"><div class="bg-white p-12 rounded-[5rem] shadow-2xl border border-slate-100 relative overflow-hidden"><h2 class="text-5xl font-black text-slate-900 mb-12 tracking-tighter italic uppercase leading-none">Solicitud <br><span class="text-brand">Auditada.</span></h2><div class="bg-slate-50 p-12 rounded-[4rem] mb-12 border border-slate-100 shadow-inner"><img src="{{ pub.imagen_url }}" class="w-48 h-48 rounded-[2.5rem] object-cover mx-auto mb-8 shadow-2xl ring-8 ring-white"><h3 class="font-black text-2xl text-slate-800 uppercase italic">{{ pub.nombre }}</h3></div>{% if pub.categoria == 'Sangre' %}<div class="mb-10 p-8 bg-red-50 border-2 border-red-100 rounded-[3rem] text-left shadow-inner italic"><p class="text-[10px] font-black text-red-600 uppercase tracking-widest mb-4 italic">Registro de Banco de Sangre</p><input name="hospital" placeholder="NOMBRE DEL HOSPITAL" required class="w-full p-5 bg-white rounded-2xl border-none text-xs font-black outline-none focus:ring-2 focus:ring-red-400 shadow-md uppercase italic"></div>{% endif %}<form action="{{ url_for('procesar_transaccion', id=pub.id_oferta_insumo) }}" method="POST"><button class="w-full btn-medical py-6 rounded-[3rem] font-black text-3xl shadow-2xl shadow-blue-200 hover:scale-105 transition-all uppercase italic">Confirmar Compromiso</button></form></div></div>{% endblock %}""",
    'review.html': """{% extends "base.html" %}{% block content %}<div class="max-w-md mx-auto py-24 px-4 text-center uppercase italic"><div class="bg-white p-16 rounded-[4.5rem] shadow-2xl border border-slate-100 italic"><h2 class="text-3xl font-black mb-12 tracking-tighter italic uppercase leading-none italic">Auditar <br><span class="text-emerald-500 text-2xl">Donante.</span></h2><form method="POST" class="space-y-8 italic"><select name="estrellas" class="w-full p-5 bg-slate-50 rounded-2xl border-none font-black text-brand text-xl text-center outline-none italic"><option value="5">★★★★★ Excelente</option><option value="4">★★★★ Bueno</option><option value="3">★★★ Regular</option><option value="2">★★ Malo</option><option value="1">★ Fraude</option></select><textarea name="comentario" placeholder="Describe tu experiencia operativa..." required rows="4" class="w-full p-6 bg-slate-50 rounded-[2rem] border-none outline-none font-bold text-sm focus:ring-2 focus:ring-emerald-400 shadow-inner italic uppercase italic"></textarea><button class="w-full bg-emerald-500 text-white py-6 rounded-[3rem] font-black text-2xl shadow-2xl shadow-emerald-100 uppercase italic transition-all hover:scale-105 italic">Publicar Score</button></form></div></div>{% endblock %}""",
    'register.html': """{% extends "base.html" %}{% block content %}<div class="max-w-2xl mx-auto py-16 px-4 uppercase italic"><div class="bg-white p-16 rounded-[4rem] shadow-2xl border border-slate-50 italic"><h2 class="text-4xl font-black text-slate-900 mb-10 tracking-tighter italic">Cédula de <span class="text-brand italic">Registro.</span></h2><form method="POST" class="grid grid-cols-1 md:grid-cols-2 gap-6 italic"><input name="nombre" placeholder="Nombre completo" required class="col-span-1 md:col-span-2 p-5 bg-slate-50 rounded-2xl border-none font-black text-xs outline-none focus:ring-2 focus:ring-brand shadow-inner uppercase italic"><select name="tipo_sangre" required class="p-5 bg-slate-50 rounded-2xl border-none font-black text-[10px] uppercase outline-none focus:ring-2 focus:ring-brand shadow-inner italic"><option value="">TIPO SANGRE</option><option>O+</option><option>O-</option><option>A+</option><option>A-</option><option>B+</option><option>B-</option><option>AB+</option><option>AB-</option></select><input name="telefono" placeholder="WhatsApp / Telegram" required class="p-5 bg-slate-50 rounded-2xl border-none font-black text-xs outline-none focus:ring-2 focus:ring-brand shadow-inner italic uppercase"><input name="email" type="email" placeholder="Email corporativo" required class="p-5 bg-slate-50 rounded-2xl border-none font-black text-xs outline-none focus:ring-2 focus:ring-brand shadow-inner italic uppercase"><input name="ubicacion" placeholder="Delegación / Ciudad" required class="p-5 bg-slate-50 rounded-2xl border-none font-black text-xs outline-none focus:ring-2 focus:ring-brand shadow-inner italic uppercase"><input name="password" type="password" placeholder="Passphrase" required class="col-span-1 md:col-span-2 p-5 bg-slate-50 rounded-2xl border-none font-black text-xs outline-none focus:ring-2 focus:ring-brand shadow-inner uppercase tracking-widest italic"><div class="col-span-1 md:col-span-2 flex items-center gap-4 bg-slate-50 p-6 rounded-3xl border border-slate-100 shadow-inner"><input type="checkbox" required class="w-6 h-6 rounded-lg text-brand border-slate-200 shadow-sm"><p class="text-[9px] font-black text-slate-400 uppercase tracking-widest italic leading-relaxed">Acepto tratamiento de datos personales LFPDPPP y confirmo uso ético de la plataforma TechPulse.</p></div><button class="col-span-1 md:col-span-2 w-full btn-medical py-6 rounded-[3rem] font-black text-2xl mt-8 shadow-2xl shadow-blue-100 italic uppercase italic">Activar Nodo</button></form></div></div>{% endblock %}""",
    'login.html': """{% extends "base.html" %}{% block content %}<div class="max-w-md mx-auto py-24 px-4 text-center uppercase italic"><div class="bg-white p-16 rounded-[5rem] shadow-2xl border border-slate-100 relative overflow-hidden shadow-blue-50/50 italic"><div class="w-24 h-24 bg-blue-50 text-brand rounded-[2rem] flex items-center justify-center mx-auto mb-12 shadow-inner rotate-6 italic"><i class="fas fa-fingerprint text-4xl"></i></div><h2 class="text-5xl font-black mb-12 tracking-tighter italic text-slate-800 uppercase leading-none italic">Validar <br>Acceso.</h2><form method="POST" class="space-y-6 italic"><input name="email" type="email" placeholder="USER ID" required class="w-full p-6 bg-slate-50 rounded-3xl border-none font-black text-xs outline-none focus:ring-2 focus:ring-brand shadow-inner uppercase tracking-widest italic"><input name="password" type="password" placeholder="PASSPHRASE" required class="w-full p-6 bg-slate-50 rounded-3xl border-none font-black text-xs outline-none focus:ring-2 focus:ring-brand shadow-inner uppercase tracking-widest italic"><button class="w-full btn-medical py-7 rounded-[3.5rem] font-black text-3xl mt-10 shadow-2xl shadow-blue-200 uppercase tracking-tighter italic italic">Ingresar</button></form></div></div>{% endblock %}""",
    'perfil.html': """{% extends "base.html" %}{% block content %}<div class="max-w-3xl mx-auto py-20 px-4 text-center uppercase italic"><div class="relative inline-block mb-12 italic"><div class="w-48 h-48 bg-brand text-white text-8xl font-black rounded-[4rem] flex items-center justify-center mx-auto shadow-2xl border-[12px] border-white ring-2 ring-slate-100 italic shadow-blue-100 italic">{{ current_user.nombre[0] | upper }}</div><div class="absolute -bottom-2 -right-2 bg-emerald-500 w-14 h-14 rounded-2xl flex items-center justify-center text-white border-4 border-white shadow-xl rotate-12"><i class="fas fa-certificate text-xl"></i></div></div><h2 class="text-6xl font-black text-slate-900 mb-2 tracking-tighter uppercase italic leading-none italic">{{ current_user.nombre }}</h2><p class="text-brand font-black uppercase tracking-[0.4em] text-[10px] mb-16 italic italic">Nodo Verificado {{ current_user.tipo_sangre }}</p><div class="grid grid-cols-1 md:grid-cols-3 gap-8 text-left italic">{% for label, icon, val in [('Canal', 'fa-mobile-screen', current_user.telefono), ('Email', 'fa-envelope-open', current_user.email), ('Ubicación', 'fa-map-location-dot', current_user.ubicacion)] %}<div class="bg-white p-10 rounded-[3rem] shadow-sm border border-slate-100 flex flex-col items-center text-center group hover:border-brand transition-all italic"><i class="fas {{ icon }} text-slate-100 text-3xl mb-5 group-hover:text-brand transition-colors italic"></i><p class="text-[9px] text-slate-300 font-black uppercase mb-2 tracking-[0.2em] italic italic">{{ label }}</p><p class="font-black text-slate-800 text-xs break-words tracking-tighter uppercase italic">{{ val }}</p></div>{% endfor %}</div><div class="mt-24 italic"><a href="{{ url_for('logout') }}" class="text-red-300 font-black text-[9px] uppercase tracking-[0.4em] hover:text-red-500 transition-all underline decoration-red-50 font-bold italic italic">Finalizar Sesión Operativa</a></div></div>{% endblock %}""",
    'chat.html': """{% extends "base.html" %}{% block content %}<div class="max-w-4xl mx-auto py-8 px-4 h-[80vh] flex flex-col uppercase italic"><div class="bg-white rounded-[4rem] shadow-2xl flex flex-col flex-1 overflow-hidden border border-slate-100 animate-in zoom-in-95 duration-700 shadow-blue-50/50 italic"><div class="bg-brand p-10 text-white flex justify-between items-center shadow-lg relative z-10 italic"><div class="flex items-center gap-6 italic"><div class="w-16 h-16 rounded-[1.5rem] bg-white/20 flex items-center justify-center font-black text-2xl shadow-inner italic">LL</div><div><h3 class="font-black leading-none text-2xl tracking-tighter italic uppercase italic">Línea Coordinación</h3><p class="text-[10px] text-blue-50 uppercase tracking-[0.2em] mt-3 font-black animate-pulse italic">Encriptación End-to-End Activa</p></div></div><a href="{{ url_for('dashboard') }}" class="w-12 h-12 bg-white/10 rounded-2xl flex items-center justify-center hover:bg-white/20 transition-all shadow-inner italic"><i class="fas fa-times text-xl italic"></i></a></div><div class="flex-1 overflow-y-auto p-12 space-y-10 bg-slate-50/50 custom-scrollbar italic italic" id="chat-box"></div><div class="p-10 bg-white border-t border-slate-100 italic"><form onsubmit="event.preventDefault(); send();" class="flex gap-6 italic"><input id="msg-input" placeholder="Coordina entrega..." class="flex-1 p-6 bg-slate-100 rounded-[2rem] border-none outline-none focus:ring-2 focus:ring-brand font-black text-sm shadow-inner uppercase tracking-tighter italic italic"><button class="bg-brand text-white w-20 h-20 rounded-[2rem] shadow-2xl shadow-blue-200 hover:scale-110 transition-transform flex items-center justify-center shadow-brand/20 italic"><i class="fas fa-paper-plane text-2xl italic"></i></button></form></div></div></div><script>const socket = io(); const room = "{{ solicitud.id_solicitud }}"; const user = "{{ current_user.nombre }}"; socket.emit('join', {room: room}); socket.on('nuevo_mensaje', function(data){ const box = document.getElementById('chat-box'); const isMe = data.user === user; const d = document.createElement('div'); d.className = `flex ${isMe ? 'justify-end':'justify-start'}`; d.innerHTML = `<div class="${isMe?'bg-brand text-white rounded-l-[2rem] rounded-tr-[2rem] shadow-2xl shadow-blue-100':'bg-white text-slate-700 rounded-r-[2rem] rounded-tl-[2rem] shadow-sm border border-slate-200'} px-8 py-5 max-w-[85%] animate-in fade-in slide-in-from-bottom-4"><p class="text-[9px] font-black uppercase mb-3 ${isMe?'text-blue-100':'text-slate-300'} tracking-widest italic">${data.user}</p><p class="text-sm font-black leading-relaxed tracking-tight uppercase italic">${data.msg}</p></div>`; box.appendChild(d); box.scrollTop = box.scrollHeight; }); function send(){ const i = document.getElementById('msg-input'); if(i.value.trim()){ socket.emit('enviar_mensaje', {msg: i.value, room: room}); i.value=''; } }</script>{% endblock %}"""
})
...
def buscar():
    ...
    today = date.today()
    # Filtro dinámico: Solo verificados y NO caducados
